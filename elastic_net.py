...
'''
    Code to carry out Regression Analysis using Elastic Net and Permutation Feature Importance
'''
def elastic_net_predict(X, y, fname='Untitled'):
    global row
    k_fold=10
    clf = ElasticNetCV(l1_ratio=[.1, .3, .5, .7, .9, .95, .99, 1], cv=k_fold, max_iter=10**6, n_jobs=-1).fit(X,y)
    alpha = clf.alpha_
    l1_ratio = clf.l1_ratio_
    print("l1_ratio=%.4f, alpha=%.4f"%(l1_ratio, alpha))
    best_estimator = ElasticNet(l1_ratio=l1_ratio, alpha=alpha)
    ...
    save.loc[row, :] = [dep, typ1+'_'+typ2, 'Elastic Net', "l1_ratio=%.4f, alpha=%.4f"%(l1_ratio, alpha), mae, exp_var, r2]
    row = row + 1

    perm = PermutationImportance(best_estimator, cv=k_fold, n_iter=250).fit(X, y)
    perm_df = eli5.explain_weights_df(perm, feature_names = X.columns.tolist())

    perm_df['abs_weight'] = np.abs(perm_df['weight'])
    perm_df = perm_df.sort_values(by=['abs_weight'], ascending=False).set_index('feature')

    file = 'elastic_net_results.xlsx'
